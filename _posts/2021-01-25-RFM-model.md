...
DROP TABLE IF EXISTS rfm_02;
CREATE TABLE IF NOT EXISTS rfm_02
SELECT 	b.ugyfel_id_dedupl,
		b.marketing_engedely_fl,
		b.callcenter_nyilatkozat,
		b.reklam_nyilatkozat,
		b.hirlevel, 
		coalesce(ROUND(SUM(b.evesdij*DATEDIFF(kockazatviseles_vege,kockazatviseles_kezdete)/365),0),0) AS sum_alltime_CASCO_KGFB_LAKAS_annual_fee_per_user,
		coalesce(sum(CASE WHEN lecso_statusz = 'élő' THEN b.evesdij END),0) as sum_current_CASCO_KGFB_LAKAS_annual_fee_per_user,		
		count(DISTINCT b.rendszam) AS num_of_insured_cars,
		count(DISTINCT CASE WHEN lecso_statusz = 'élő' THEN b.rendszam END) as current_num_of_insured_cars,
		case 
		when MIN(DATEDIFF(CURRENT_DATE,kockazatviseles_vege)) = 0 then 
			case 
				when KGFB_elo > 0 and CASCO_elo = 0 and LAKAS_elo = 0 then 'K'
				when KGFB_elo = 0 and CASCO_elo > 0 and LAKAS_elo = 0 then 'C'
				when KGFB_elo = 0 and CASCO_elo = 0 and LAKAS_elo > 0 then 'L'
				when KGFB_elo > 0 and CASCO_elo > 0 and LAKAS_elo = 0 then 'KC'
				when KGFB_elo > 0 and CASCO_elo = 0 and LAKAS_elo > 0 then 'KL'
				when KGFB_elo = 0 and CASCO_elo > 0 and LAKAS_elo > 0 then 'CL'
				when KGFB_elo > 0 and CASCO_elo > 0 and LAKAS_elo > 0 then 'KCL'
			end
		when MIN(DATEDIFF(CURRENT_DATE,kockazatviseles_vege)) > 0 then 'törölt'
		end	as recency_cat,
		DATEDIFF(MAX(kockazatviseles_vege),MIN(kockazatviseles_kezdete)) AS length_of_relatonship_day,
		case 
		when utolso_csatorna = 'online' then 'online'
		else 'nem online'
		end	as utolso_csatorna_cat
FROM rfm_01 b
GROUP BY b.ugyfel_id_dedupl
;
