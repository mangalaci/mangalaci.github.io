...
- I extracted the contract table from the company's relational database. (Customers identities are deduplicated in the system already.) 

<br>
Filtering:
-	MTPL, CASCO, Home inssurance contracts
...
kalkulált mezők:
-	sum_alltime_CASCO_KGFB_LAKAS_annual_fee_per_user: az utolsó ismert éves díj felszorozva a kockázatviselés kezdete és vége között eltelt egész évek számával. Ha még jelenleg élő szerződésről van szó, akkor a kockázatviselés vége megegyik az aznapi dátummal;
-	sum_current_CASCO_KGFB_LAKAS_annual_fee_per_user: élő szerződés esetében az utolsó ismert éves díj összege;
-	current_num_of_insured_cars: a jelenleg is élő biztosítással rendelkező gépjárművek száma (csak az számít gépjárművnek itt, aminek rendszáma van);
-	length_of_relatonship_day: kockázatviselés kezdete és vége között eltelt napok száma;
-	recency_cat: milyen élő szerződés(ek)kel rendelkezik az adott ügyfél: lehetséges esetek = 1. csak KGFB, 2. csak CASCO, 3. csak LAKÁS, 4. KGFB és CASCO, 5. KGFB és LAKÁS, 6. CASCO és LAKÁS, 7. illetve mind a 3 biztosítással rendelkezik, 8. nem rendelkezik élő szerződéssel.
-	

extrém értékek kizására:
-	2.000.000 Ft feletti kumulált éves díjakkal rendelkező ügyfelek törlése;
-	10.000 nap feletti ügyfélviszonnyal rendelkező ügyfelek törlése;
